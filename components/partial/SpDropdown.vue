<template>
  <div class="sp-dropdown relative" @mouseover="open = true" @mouseleave="open = false">
    ...
    >
      <div
        v-if="open"
        class="content hidden absolute shadow-md border-t-4 border-yellow-400 rounded-md text-sm bg-white md:block"
    ...
    >
      <div
        v-if="open"
        class="content absolute shadow-md border-t-2 border-yellow-400 rounded-md text-sm bg-white md:hidden"
        :class="placement === 'right' ? 'right-0' : 'left-0'"
        style="z-index: 1;"
        :style="width === 'lg' ? 'width: 24rem;' : ''"
      >
...
<style lang="less">
  .sp-dropdown {
    ...
    &:not(.clear) .content {
      @apply w-60 py-1;
      .content-item {
        @apply block duration-300 cursor-pointer hover:bg-yellow-400 hover:bg-opacity-25;
      }
    }
  }
</style>
